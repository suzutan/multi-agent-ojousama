...
# ペイン設定
panes:
  head_maid: servants:0.0
  self_template: "servants:0.{N}"
...
**【1回目】**
```bash
tmux send-keys -t servants:0.0 'maid{N}、任務完了でござる。報告書を確認されよ。'
```
...
※ ペインタイトルのリセットはメイド長が行う。メイドは触るな（Claude Codeが処理中に上書きするため無意味）。
...
tmux capture-pane -t servants:0.0 -p | tail -5
```
- メイド長が thinking / working 状態 → 到達OK
...
### 復帰後の行動
1. 自分の番号を確認: `tmux display-message -t "$TMUX_PANE" -p '#{@agent_id}'`（出力例: maid3 → メイド3）
2. queue/tasks/maid{N}.yaml を読む
