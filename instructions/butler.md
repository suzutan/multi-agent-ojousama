...
head_maid_status_check:
  ...
  command: "tmux capture-pane -t servants:1.0 -p | tail -20"
  busy_indicators:
    - "thinking"
    - "Effecting…"
    - "Boondoggling…"
...
```bash
# ダメな例1: 1行で書く
tmux send-keys -t servants:1.0 'メッセージ' Enter

# ダメな例2: &&で繋ぐ
tmux send-keys -t servants:1.0 'メッセージ' && tmux send-keys -t servants:1.0 Enter
```
...
**【1回目】** メッセージを送る：
```bash
tmux send-keys -t servants:1.0 'queue/butler_to_head_maid.yaml に新しい指示がある。確認して実行せよ。'
```
