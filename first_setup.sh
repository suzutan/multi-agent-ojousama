#!/bin/bash
# ============================================================
# first_setup.sh - multi-agent-ojousama 初回セットアップスクリプト
...
echo ""
echo "  ╔══════════════════════════════════════════════════════════════╗"
echo "  ║  🏯 multi-agent-ojousama インストーラー                         ║"
...
# multi-agent-ojousama 設定ファイル

# 言語設定
# ja: 日本語（貴族邸宅風日本語のみ、併記なし）
# en: 英語（貴族邸宅風日本語 + 英訳併記）
...
# スキル設定
skill:
  # スキル保存先（スキル名に ojousama- プレフィックスを付けて保存）
...
# ============================================================
# STEP 8: メイド用タスク・レポートファイル初期化
# ============================================================
log_step "STEP 8: キューファイル初期化"

# メイド用タスクファイル作成
for i in {1..8}; do
    TASK_FILE="$SCRIPT_DIR/queue/tasks/maid${i}.yaml"
    if [ ! -f "$TASK_FILE" ]; then
        cat > "$TASK_FILE" << EOF
# メイド${i}専用タスクファイル
task:
...
log_info "メイドタスクファイル (1-8) を確認/作成しました"

# メイド用レポートファイル作成
for i in {1..8}; do
    REPORT_FILE="$SCRIPT_DIR/queue/reports/maid${i}_report.yaml"
...
log_info "メイドレポートファイル (1-8) を確認/作成しました"
...
# css alias (執事長ウィンドウの起動)
if [ -f "$BASHRC_FILE" ]; then
    EXPECTED_CSS="alias css='tmux attach-session -t lady'"
    if ! grep -q "alias css=" "$BASHRC_FILE" 2>/dev/null; then
        # alias が存在しない → 新規追加
        echo "" >> "$BASHRC_FILE"
        echo "# multi-agent-ojousama aliases (added by first_setup.sh)" >> "$BASHRC_FILE"
        echo "$EXPECTED_CSS" >> "$BASHRC_FILE"
        log_info "alias css を追加しました（執事長ウィンドウの起動）"
        ALIAS_ADDED=true
    ...
    # csm alias (メイド長・メイドウィンドウの起動)
    EXPECTED_CSM="alias csm='tmux attach-session -t servants'"
    if ! grep -q "alias csm=" "$BASHRC_FILE" 2>/dev/null; then
        ...
        log_info "alias csm を追加しました（メイド長・メイドウィンドウの起動）"
        ALIAS_ADDED=true
...
if command -v claude &> /dev/null; then
    ...
    if claude mcp list 2>/dev/null | grep -q "memory"; then
        ...
        if claude mcp add memory \
            -e MEMORY_FILE_PATH="$SCRIPT_DIR/memory/lady_memory.jsonl" \
