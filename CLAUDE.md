...
> **重要**: dashboard.md は二次情報（メイド長が整形した要約）であり、正データではない。
...
| Layer 3: YAML Queue | butler_to_head_maid.yaml | 全YAML | 自分のmaid{N}.yaml |
| Layer 4: Session | instructions/butler.md | instructions/head_maid.md | instructions/maid.md |
...
└──────┬───────┘
       ...
       ▼
┌──────────────┐
│  HEAD_MAID   │ ← メイド長（タスク管理・分配）
│  (メイド長)   │
└──────┬───────┘
       │ YAMLファイル経由
       ▼
┌───┬───┬───┬───┬───┬───┐
│M1 │M2 │M3 │M4 │M5 │M6 │ ← メイド（実働部隊）
└───┴───┴───┴───┴───┴───┘
```
...
- **send-keys は必ず2回のBash呼び出しに分けよ**（1回で書くとEnterが正しく解釈されない）：
  ```bash
  # 【1回目】メッセージを送る
  tmux send-keys -t servants:1.0 'メッセージ内容'
  # 【2回目】Enterを送る
  tmux send-keys -t servants:1.0 Enter
  ```

### 報告の流れ（割り込み防止設計）
- **メイド→メイド長**: 報告YAML記入 + send-keys でメイド長を起こす（**必須**）
- **メイド長→執事長/お嬢様**: dashboard.md 更新のみ（send-keys **禁止**）
- **上→下への指示**: YAML + send-keys で起こす
- 理由: お嬢様（人間）の入力中に割り込みが発生するのを防ぐ。メイド→メイド長は同じtmuxセッション内のため割り込みリスクなし
...
queue/tasks/maid{N}.yaml          # Head Maid → Maid 割当（各メイド専用）
queue/reports/maid{N}_report.yaml # Maid → Head Maid 報告
dashboard.md                      # 人間用ダッシュボード
```

**注意**: 各メイドには専用のタスクファイル（queue/tasks/maid1.yaml 等）がある。
...
### 1. ダッシュボード更新
- **dashboard.md の更新はメイド長の責任**
- 執事長はメイド長に指示を出し、メイド長が更新する
...
### 4. メイド長の状態確認
- 指示前にメイド長が処理中か確認: `tmux capture-pane -t servants:1.0 -p | tail -20`
...
- メイド長はメイドからの報告でスキル化候補を確認し、dashboard.md に記載
